...
const TimePickerModal = ({ visible, onClose, onSelect, initialHour = 9, title = 'Select Time' }) => {
	...
	// Reset selectedHour when modal opens or initialHour changes
	useEffect(() => {
		if (visible) {
	...
	return (
		<Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
			<Pressable style={commonStyles.modalContainer} onPress={onClose}>
				<Pressable style={[commonStyles.modalContent, { height: 300 }]} onPress={(e) => e.stopPropagation()}>
					...
					>
						<TouchableOpacity style={commonStyles.secondaryButton} onPress={onClose}>
