import { sendPushNotification, scheduleLocalNotificationWithPush } from '../../utils/notifications/pushNotification';
...
describe('Push Notification System', () => {
    ...
    describe('sendPushNotification', () => {
        ...
        it('should filter out null tokens', async () => {
					...
					expect(result).toBeTruthy(); // Should succeed but not make the API call
					expect(global.fetch).not.toHaveBeenCalled();
					expect(getDoc).toHaveBeenCalled(); // Verify check for token
				});
    ...
    describe('scheduleLocalNotificationWithPush', () => {
        it('should schedule both local and push notifications for future events', async () => {
            ...
            const localNotificationId = await scheduleLocalNotificationWithPush(
                'user1',
                content,
                trigger
            );
        ...
        it('should handle scheduling errors', async () => {
            ...
            await expect(
                scheduleLocalNotificationWithPush('user1', content, trigger)
